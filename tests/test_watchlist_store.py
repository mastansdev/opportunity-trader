...
def test_the_panel_shows_all_four_groups():
    page = _board()
    for label in ("REPORTING DURING MARKET", "REPORTING AFTER CLOSE",
                  "TIME NOT KNOWN", "YOU ADDED"):
        assert label in page, label


def test_the_rows_carry_qty_and_a_buy_button():
    """---- ONE ACTION, NOT TWO. ----

    This asserted actBtn(sym, "SELL") beside BUY, because the old page
    had a SHORT button on every watchlist row.

        "Opportunity Trader Bot = ... only long positions."

    /board has never had a short action anywhere on it, and restoring
    a deleted panel is not the moment to introduce one. Qty and BUY,
    the same two controls the movers table uses, on the same endpoint.
    """
    page = _board()
    block = page[page.find("function wlRows("):page.find("function drawWatch(")]
    assert block, "wlRows is gone -- the watchlist rows are not being built"
    assert "data-qtyfor=" in block
    assert "data-buy=" in block
    assert 'data-act="short"' not in block, (
        "a SHORT button on a long-only bot's watchlist")


def test_only_his_own_rows_can_be_removed():
    """The results groups are recomputed from the calendar every
    session. A remove button on them would suggest an edit that the
    next refresh would silently undo."""
    page = _board()
    block = page[page.find("function drawWatch("):page.find("let NAMES")]
    assert block, "drawWatch is gone"
    assert "w.mine, true)" in block
    assert "w.during, false)" in block
    assert "w.after, false)" in block
    assert "w.unknown, false)" in block


def test_the_watch_tab_is_wired_into_the_pane_switcher():
    """     "WATCHLIST MOVE THIS CHIP TO TOP ABOVE FNO CHIP IN SIDE BAR"

    The sidebar of chips that request was about belonged to screen.html
    and went with it; /board uses tabs. So the requirement it stands
    for now is simply that the tab EXISTS and SWITCHES.

    That second half is not a formality. The tab handler refuses any
    name not in its own list, after a sort button borrowing class="tab"
    blanked the whole page on 11 August -- so a tab added to the markup
    and forgotten in the list is a button that does nothing at all.
    """
    page = _board()
    assert 'data-tab="watch"' in page, "no Watch tab on the board"
    assert 'id="watch-pane"' in page, "the Watch tab has no pane to show"
    ...
    assert '"watch"' in block, (
        "the Watch tab is in the markup but not in the switcher's list of "
        "allowed panes, so clicking it does nothing")
    assert block.count('"watch"') >= 2, (
        "the pane must be named in BOTH the allow-list and the show/hide "
        "loop, or the tab highlights and no pane appears")
