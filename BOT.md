<!-- generated: 2026-09-04 08:44 by tools/bot_doc.py -- do not edit by hand -->
...
|---|---|---|
| Your own margin per position | ₹50,000 | `config.MTF_MARGIN_PER_POSITION_RS` |
| MTF leverage | 4× | `config.MTF_LEVERAGE` |
| Stock value per position | ≈₹200,000 | derived |
| Risk budget per trade | ₹2,500 | **`core/rules.py`** — the only owner |
| Hard stop from entry | 2.50% | `config.HARD_STOP_FROM_ENTRY_PCT` |
| A stop-out therefore costs | ≈₹5,000 | derived |
| Stop for the day after losing | ₹12,000 (≈2 stop-outs) | `config.DAILY_MAX_LOSS_RS` |
